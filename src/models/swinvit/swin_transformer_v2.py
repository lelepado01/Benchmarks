...
def swin_transformer_v2_t(input_resolution: [int, int],
                          window_size: int = 7,
                          in_channels: int = 3,
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        max_t = torch.log(torch.tensor(1. / 0.01)).to(self.logit_scale.device)
        
        logit_scale = torch.clamp(self.logit_scale, max=max_t).exp()
